...
public class Cat_Auto_Auditoria extends JDialog{
	
	Container cont = getContentPane();
	JLayeredPane panel = new JLayeredPane();
	
	Obj_Auto_Auditoria autori = new Obj_Auto_Auditoria().buscar();
	boolean autorizar = autori.isAutorizar();
	
	JLabel lblAutorizar = new JLabel(new ImageIcon("imagen/Aplicar.png"));
	
	public Cat_Auto_Auditoria(){
		...
		public void mousePressed(MouseEvent arg0) {
			...
			if(autorizar != true){
				if(JOptionPane.showConfirmDialog(null, "¿Desea autorizar la lista de raya?") == 0){
					Obj_Auto_Auditoria prue = new Obj_Auto_Auditoria();
					autorizar = true;
